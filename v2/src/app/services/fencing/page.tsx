import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "Fencing | Waterstone Companies",
  description: "Privacy, security, and curb appeal. Wood, vinyl, aluminum, and chain-link fencing.",
};

export default function FencingPage() {
  return (
    <section className="py-24 px-6">
      <div className="max-w-3xl mx-auto">
        <p className="text-label uppercase tracking-widest text-foreground-muted mb-3">Services</p>
        <h1 className="font-serif text-h1 text-foreground">Fencing</h1>
        <p className="mt-6 text-body-lg text-foreground-muted">
          Port content from existing services/fencing/index.html.
        </p>
      </div>
    </section>
  );
}
