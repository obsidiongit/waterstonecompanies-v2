import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "Interior Accents | Waterstone Companies",
  description: "Shiplap, wainscoting, and trim work to elevate your home interior.",
};

export default function InteriorAccentsPage() {
  return (
    <section className="py-24 px-6">
      <div className="max-w-3xl mx-auto">
        <p className="text-label uppercase tracking-widest text-foreground-muted mb-3">Services</p>
        <h1 className="font-serif text-h1 text-foreground">Interior Accents</h1>
        <p className="mt-6 text-body-lg text-foreground-muted">
          Port content from existing services/interior-accents/index.html.
        </p>
      </div>
    </section>
  );
}
